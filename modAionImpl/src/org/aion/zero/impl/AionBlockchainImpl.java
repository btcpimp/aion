...
//        }
        ...
        if (!block.isGenesis()) {
            ...
            if (txs != null && !txs.isEmpty()) {
                ...
                if (!Arrays.equals(getBlockStore().getBestBlock().getHash(), block.getParentHash())) {
                    AionBlock parentBlock = getBlockByHash(block.getParentHash());
                    byte[] stateRoot = parentBlock.getStateRoot();

                    parentRepo = repository.getSnapshotTo(stateRoot);

                    if (!parentRepo.isValidRoot(stateRoot)) {
                        if (repository.isSnapshot()) {
                            recoverWorldState(stateStack.peek().savedRepo, parentBlock);
                        } else {
                            recoverWorldState(repository, parentBlock);
                        }
                    }
                }
    ...
    public synchronized boolean recoverWorldState(IRepository repository, AionBlock block) {
        ...
        LOG.info("Rebuilding from block hash = {}, number = {}.", other.getShortHash(), other.getNumber());
